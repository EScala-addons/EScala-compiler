...
object Test
{
  ...
  evt e5 = e3 && causedBy(e1)
    ...
    // register reaction to event 3, 4 and 5
    e3 += ((v: Int, proceed: Int => Unit) => println("reaction of e3 = e1 || e2 override proceed, v = " + v))
    e4 += ((v: Int, proceed: Int => Unit) =>
      {
        println("e4 = e3 && ((x:Int) => x = 3) calls proceed first")
        proceed(v)
        println("reaction of event 4, v = " + v)
      })
    e5 += ((v: Int, proceed: Int => Unit) =>
      {
        println("e5 = e3 && causedBy(e1)")
        println("reaction of event 5, v = " + v)
