...
trait ParsersCommon extends ScannersCommon {
  ...
  trait ParserCommon {
    ...
    def inBracketsOrError[T](body: => T, alt: T): T =
      if (in.token == LBRACKET) inBrackets(body)
      else { accept(LBRACKET) ; alt }
...
/* -------- MODIFIERS and ANNOTATIONS ------------------------------------------- */    
    ...
    private val flagTokens: Map[Int, Long] = Map(
      ...
      SEALED    -> Flags.SEALED,
      OBSERVABLE-> (Flags.OBSERVABLE | Flags.INSTRUMENTED),
    ...
    def accessModifierOpt(): Modifiers = normalize {      
      in.token match {
        // @ESCALA
        case m @ (PRIVATE | PROTECTED | OBSERVABLE)  => in.nextToken() ; accessQualifierOpt(Modifiers(flagTokens(m)))
        case _                          => NoMods
      }
    }
    ...
    def modifiers(): Modifiers = normalize {
      def loop(mods: Modifiers): Modifiers = in.token match {
        // @ESCALA
        case PRIVATE | PROTECTED | OBSERVABLE | IMPERATIVE =>
    ...
    def eventDefOrDcl(start : Int, mods: Modifiers): Tree = {
      ...
      atPos(start, if (name == nme.ERROR) start else nameOffset) {
        val types: Option[List[Tree]] =
          if(in.token == LBRACKET)  
            Some(inBracketsOrError(commaSeparated(exprSimpleType()),
              List(errorTermTree)))
          else 
            None
